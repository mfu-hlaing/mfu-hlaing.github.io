...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (preloader) {
        const msgs = ['Heating the pan… 🔥', 'Mise en place…', 'Adding a pinch of love… 💘', 'Service! 🔥💘'];
        let prog = 0, mi = 0;
    ...
    if (particlesContainer && window.innerWidth > 768) {
        for (let i = 0; i < 30; i++) {
            ...
            p.style.width = s + 'px';
            p.style.height = s + 'px';
            const colors = ['#f0c860','#ffe08a','#ff5e3a','#ff5d8f','#ff7a52','#c79a2e'];
        ...
        for (let i = 0; i < 50; i++) {
            ...
            star.style.background = Math.random() > 0.7 ? '#f0c860' : '#f5ead2';
    ...
    if (tw) {
        const phrases = [
            ' a dish made with love, just for you 💕',
            ' Diable Jambe — too hot to handle 🔥',
            " I'd never let a lady go hungry 🥰",
            ' plated to perfection 🍳',
            ' mellorine~! 😍',
            ' kicking bugs, never the cook\'s hands 🦵',
            ' sailing for All Blue (and love) 🌊💘',
        ];
    ...
    document.addEventListener('click', e => {
        for (let i = 0; i < 5; i++) {
            ...
            spark.style.cssText = `
                ...
                background:${['#ff5e3a','#f0c860','#ff5d8f','#ffe08a'][Math.floor(Math.random()*4)]};
